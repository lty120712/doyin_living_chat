...
function mediaStyle(m, size) {
  const base = { borderRadius: guestBorderRadius(size) + 'px', borderWidth: guestBorderWidth(size) + 'px' }
...
<template>
  <div class="guest" v-for="(g, i) in state.guests" :key="i"
    ...
    <video v-if="g.media?.type === 'video'" :src="g.media.url" muted loop class="guestMedia"
    ...
    <video v-else autoplay muted loop src="" class="guestPlaceholder"
